...
def update_final_logos(offset):
    try: 
        ...
        while True:
            ...
            dates_as_datetime = [datetime.strptime(date, "%Y-%m-%dT%H:%M:%S") for date in post_published]
            max_date = max(dates_as_datetime).isoformat()
            data, count = supabase.table('t_h02_article_groups_l2_detail').update({'logo_urls': logoUrls, 'outlets': outlets, 'image_urls': imageUrls, 'post_published': max_date}).filter('article_group_id', 'eq', id).execute()
        end_date = datetime.now(timezone.utc).replace(microsecond=0)
        end_date_str = end_date.isoformat()
        data1 = {"success": True, "end_date": end_date_str}
        data, count = supabase.table('t_w01_python_run').update(data1).eq('id', get_id).execute()
