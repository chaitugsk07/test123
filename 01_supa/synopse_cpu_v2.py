...
LIMIT 10 OFFSET """+ str(offset1) +""";"""
  while True:
    ...
    for i in range(0,len(article_ids)):
      v11 = []
      v11.append(article_ids[i])
      v11.append(str(embeddings[i].tolist()))
...
def vectorize_tam(offset1):
  print(offset1)
  model = SentenceTransformer('l3cube-pune/tamil-sentence-bert-nli')
  tovetorize = f"""SELECT
    t_v1_rss1_articles.id,
    t_v1_rss1_articles.title,
    t_v1_rss1_articles.summary as s1,
...
def set_publish_date():
    ...
    while True:
        ...
        if len(articles_l2_output) == 0:
            break
        for i in range(len(articles_l2_output)):
            update_query = "UPDATE synopse_articles.t_v4_article_groups_l2_detail SET post_published = '"+ datetime.now().astimezone(timezone.utc).isoformat().strftime('%Y-%m-%d %H:%M:%S') +"' WHERE id = " + str(articles_l2_output[i][0]) + ";"
            # print(update_query)
            update_conn_source(update_query)
...
def create_latest_8hours():
    ...
    if latest_8hours_output[0][0] > 0:
        ...
        filename = "trending_" + str(date_int) + ".wav"
        system_prompt = """
