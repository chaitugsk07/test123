...
def create_conn_source(select_query):
    ...
    try:
        conn = psycopg2.connect(
            host="ep-still-hat-a195lqjj.ap-southeast-1.aws.neon.tech",
            database="neondb",
            user="neondb_owner",
            password="UPEwCcuev9A3"
        )
        cur = conn.cursor()
...
def create_conn_source2(select_query):
    ...
    try:
        conn = psycopg2.connect(
            host="ep-spring-pine-a1906lg2.ap-southeast-1.aws.neon.tech",
            database="neondb",
            user="neondb_owner",
            password="4QbNXc9vlAYi"
        )
        cur = conn.cursor()
        cur.execute(select_query)
        rows = cur.fetchall()
...
def vectorize(offset1):
  ...
  tovetorize = f"""SELECT
    ...
    t_v1_rss1_articles.post_published
FROM
...
LIMIT 10 OFFSET """+ str(offset1) +""";"""
  while True:
    ...
    for i in range(len(tovetorize_output)):
      ...
      v11.append(tovetorize_output[i][5])
      v10.append(tuple(v11))
    v15 = tuple(v10)
    v1_insert_query ="INSERT INTO synopse_articles.t_v2_articles_vectors (article_id, a_vector, language,post_published) VALUES (%s, %s, %s, %s)"
    insert_conn_source2(v1_insert_query, v15)
...
def grouping_l1(offset1):
  ...
  while True:
    ...
    if len(article_ids) == 1:
      update_query = "UPDATE synopse_articles.t_v2_articles_vectors SET is_grouped = 1 WHERE article_id = " + str(article_ids[0]) + ";"
      update_conn_source2(update_query)
    else:
      t_ids = tuple(article_ids)
      update_query = "UPDATE synopse_articles.t_v2_articles_vectors SET is_grouped = 1 WHERE article_id in " + str(t_ids) + ";"
      update_conn_source2(update_query)
...
def grouping_l2():
  ...
  l1_q = f"""SELECT
      ...
      a.language
  FROM
      synopse_articles.t_v3_article_groups_l1 a, synopse_articles.t_v2_articles_vectors b
  WHERE
  ...
  WHERE
    ...
    while True:
      if(len(unique_elements)> 1):
        q2123 = f"""SELECT
                      ...
                      WHERE
                          article_id  IN  """ + str(tuple(unique_elements)) + """;"""
      if(len(unique_elements) == 1):
        q2123 = f"""SELECT
                          article_id,
                          is_grouped
    ...
    if len(q51_output2) == 1:
      ...
      if (len(articles_ids) > 1):
        ...
        update_conn_source2(update_query)
      elif (len(articles_ids) == 1):
        update_query = "UPDATE synopse_articles.t_v2_articles_vectors SET is_grouped = 2 WHERE article_id = " + str(t_ids[0]) + ";"
        update_conn_source2(update_query)
    else:
      v1=[]
...
def delete_article_detail():
    ...
    mutation_query = """
    DELETE FROM synopse_articles.t_v3_article_groups_l1
WHERE article_id IN (
    SELECT t_v3_article_groups_l1.article_id
    FROM synopse_articles.t_v3_article_groups_l1
    JOIN synopse_articles.t_v2_articles_vectors ON t_v3_article_groups_l1.article_id = t_v2_articles_vectors.article_id
    WHERE t_v2_articles_vectors.post_published < (now() - '15 days' :: interval)
);"""
    update_conn_source2(mutation_query)
    mutation_query = """DELETE FROM  synopse_articles.t_v2_articles_vectors
    WHERE t_v2_articles_vectors.post_published < (now() - '15 days' :: interval);"""
    update_conn_source2(mutation_query)
    mutation_query = """DELETE FROM synopse_articles.t_v2_articles_summary
WHERE article_id IN (
    select t_v1_rss1_articles.id as article_id FROM  synopse_articles.t_v1_rss1_articles
    WHERE t_v1_rss1_articles.post_published < (now() - '30 days' :: interval)
);"""
    update_conn_source(mutation_query)
    mutation_query = """DELETE FROM  synopse_articles.t_v1_rss1_articles
    WHERE t_v1_rss1_articles.post_published < (now() - '15 days' :: interval);"""
    update_conn_source(mutation_query)
...
if __name__ == "__main__":
        ...
        argsss = outlets
        n = 10
        for i in range(0, len(argsss),  n):
            argss.append(argsss[i:i+ n])
        print(argss)
        processes = []
        for args in argss:
            for arg in args:
        ...
        processes = []
        for arg in args:
            process = multiprocessing.Process(target=gen_final_article_tel, args=(arg,))
            processes.append(process)
            process.start()
        for process in processes:
            process.join()
        ...
        processes = []
        for arg in args:
            process = multiprocessing.Process(target=gen_final_article_tam, args=(arg,))
            processes.append(process)
            process.start()
        for process in processes:
            process.join()
        ...
        processes = []
        for arg in args:
            process = multiprocessing.Process(target=gen_final_article_hin, args=(arg,))
            processes.append(process)
            process.start()
        for process in processes:
            process.join()
