...
class TagAllBloc extends Bloc<TagAllEvent, TagAllState> {
  ...
  FutureOr<void> _onTagAllFetch(
    ...
    if (state.status == TagAllStatus.initial) {
      _currentPage = 0;
      if (event.isImageAvilable) {
        final articles =
            await _rssFeedServices.gettagAllSideScroll(20, 0, event.tag);
        return emit(
          state.copyWith(
    ...
    if (event.isImageAvilable) {
      final articles = await _rssFeedServices.gettagAllSideScroll(
          10, 10 + 10 * _currentPage, event.tag);
      if (articles.isEmpty) {
      ...
      final articles = await _rssFeedServices.gettagAll(
          10, 10 + 10 * _currentPage, event.tag);
      if (articles.isEmpty) {
        return emit(state.copyWith(hasReachedMax: true));
      } else {
  ...
  FutureOr<void> _onTagAllRefresh(
    ...
    add(TagAllFetch(tag: event.tag, isImageAvilable: event.isImageAvilable));
  }
}
