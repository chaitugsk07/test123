...
class RssFeedServicesFeed {
  ...
  Future<List<SynopseArticlesTV4ArticleGroupsL2Detail>> gettagAll(
    ...
    query MyQuery(\$ai_tags: [String!]!, \$limit: Int!, \$offset: Int!) {
      synopse_articles_t_v4_article_groups_l2_detail(order_by: {created_at: desc}, where: {ai_tags: {_contains: \$ai_tags}}, limit: \$limit, offset: \$offset) {
      article_group_id
      title
      image_urls
      logo_urls
  ...
  Future<List<SynopseArticlesTV4TagsHierarchyTree>> getTreeTags(
    ...
    const String query = '''
    query MyQuery(\$tag_hierachy: bigint = "") {
  ...
  Future<List<SynopseRealtimeGetEmbedding>> getSearchID(
    String search,
  ) async {
    const String query = '''
    ...
    if (response is Success) {
      final data = response.data?['synopse_realtime_get_embedding'] as List;
      log("SUCCESS DATA:${response.data.toString()}");
      final searchArticles = data
          .map((dynamic e) =>
              SynopseRealtimeGetEmbedding.fromJson(e as Map<String, dynamic>))
          .toList();
      return searchArticles;
    } else {
      return [];
